...
Chrome 扩展和油猴脚本使用相同版本号，当前版本为 `0.1.6`。
...
- 两个版本都只在 `https://xk.nju.edu.cn/xsxkapp/*` 注入评价界面；访问红黑榜网站时仅在 `<html>` 写入当前版本和来源标记，供网站判断助手是否已安装。
- Chrome 扩展的主机权限和油猴脚本的 `@connect` 都只允许访问 `njuclass.zcec.top` 的公开课程与评价接口。
- 不读取选课账号、登录令牌、课表或红黑榜登录信息，也不保存个人数据。

## 网站安装状态

访问红黑榜网站时，助手会在 `<html>` 写入 `data-njuclass-helper-version` 和 `data-njuclass-helper-source`，并派发可冒泡的 `njuclass-helper-ready` 事件。网站应先读取属性，再监听事件处理稍后注入的情况；这些标记只用于界面提示，不应作为安全凭证。
